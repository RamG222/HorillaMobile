...
import '/checkin_checkout/checkin_checkout_views/stopwatch.dart';
...
class _CheckInCheckOutFormPageState extends State<CheckInCheckOutFormPage> {
  ...
  Future<void> getLoginEmployeeWorkInfoRecord(
    String requestsEmpMyWorkInfoId,
  ) async {
    final prefs = await SharedPreferences.getInstance();
    var token = prefs.getString("token");
    var typedServerUrl = prefs.getString("typed_url");
                  ...
                  if (details.delta.dx < 0 && clockCheckedIn) {
                    ...
                    if (result != null && result['checkedOut'] == true) {
                      setState(() {
                        ...
                        clockCheckBool = false;
                        DateTime now = DateTime.now();
                        checkOutFormattedTime = DateFormat(
                          'h:mm a',
                        ).format(now);
                        swipeDirection = 'Swipe to Check-In';
                        _saveClockState(
                    ...
                    if (result != null && result['checkedIn'] == true) {
                      setState(() {
                        ...
                        checkInFormattedTimeTopR = DateFormat(
                          'h:mm',
                        ).format(now);
                        _saveClockState(
                          clockCheckedIn,
...
class User extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback(
      (_) => Navigator.pushNamed(context, '/user'),
    );
    return Container(
      color: Colors.white,
      child: const Center(child: Text('Page 1')),
